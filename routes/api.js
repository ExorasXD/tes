...
var creatorList = ['@Ryan','@ryangans','@ryan_ajalah','@ryananjay', '@ryanganssss','@ryanganteng','@ryangantengg'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6288299758728 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${ryangans}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'ExorasXD') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'ExorasXD')  return res.json(loghandler.invalidKey)
...
router.get('/infocuaca/bandara', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'ExorasXDBot') return res.json(loghandler.invalidKey)
